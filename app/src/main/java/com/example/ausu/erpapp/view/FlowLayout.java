...
//		});
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mCount == 0) {
                ...
                child.measure(MeasureSpec.makeMeasureSpec((sizeWidth - (mCount + 1) * mHorizontalSpacing) / mCount, MeasureSpec.EXACTLY), childHeightMeasureSpec);
            }
            if (mLine == null) {
                mLine = new Line();
            }
            int childWidth = child.getMeasuredWidth();
            mUsedWidth += childWidth;// 增加使用的宽�?
            if (mUsedWidth <= sizeWidth) {// 使用宽度小于总宽度，该child属于这一行�??
                mLine.addView(child);// 添加child
                mUsedWidth += mHorizontalSpacing;// 加上间隔
                if (mUsedWidth >= sizeWidth) {// 加上间隔后如果大于等于�?�宽度，�?要换�?
                    if (!newLine()) {
                        break;
                    }
                }
            } else {// 使用宽度大于总宽度�?�需要换�?
                if (mLine.getViewCount() == 0) {// 如果这行�?个child都没有，即使占用长度超过了�?�长度，也要加上去，保证每行都有至少有一个child
                    mLine.addView(child);// 添加child
                    if (!newLine()) {// 换行
                        break;
                    }
                    ...
                    // 在新的一行，不管是否超过长度，先加上去，因为这一行一个child都没有，�?以必须满足每行至少有�?个child
                    mLine.addView(child);
        ...
        if (mLine != null && mLine.getViewCount() > 0 && !mLines.contains(mLine)) {
            // 由于前面采用判断长度是否超过�?大宽度来决定是否换行，则�?后一行可能因为还没达到最大宽度，�?以需要验证后加入集合�?
            mLines.add(mLine);
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (!mNeedLayout || changed) {//没有发生改变就不重新布局
            mNeedLayout = false;
            int left = getPaddingLeft();//获取�?初的左上�?
            int top = getPaddingTop();
            final int linesCount = mLines.size();
            for (int i = 0; i < linesCount; i++) {
                final Line oneLine = mLines.get(i);
                oneLine.layoutView(left, top);//布局每一�?
                top += oneLine.mHeight + mVerticalSpacing;//为下�?行的top赋�??
            }
        }
    }
    ...
    /**
     * 代表�?�?行，封装了一行所占高度，该行子View的集合，以及�?有View的宽度�?�和
     */
    class Line {
        int mWidth = 0;// 该行中所有的子View累加的宽�?
        int mHeight = 0;// 该行中所有的子View中高度的那个子View的高�?
        List<View> views = new ArrayList<View>();

        public void addView(View view) {// �?该行中添加一�?
            views.add(view);
            mWidth += view.getMeasuredWidth();
            int childHeight = view.getMeasuredHeight();
            mHeight = mHeight < childHeight ? childHeight : mHeight;//高度等于�?行中�?高的View
        }
        ...
        public void layoutView(int l, int t) {// 布局
            ...
            if (surplusWidth >= 0) {// 剩余空间
                // 采用float类型数据计算后四舍五入能减少int类型计算带来的误�?
                int splitSpacing = (int) (surplusWidth / count + 0.5);
                for (int i = 0; i < count; i++) {
                    final View view = views.get(i);
                    int childWidth = view.getMeasuredWidth();
                    int childHeight = view.getMeasuredHeight();
                    //计算出每个View的顶点，是由�?高的View和该View高度的差值除�?2
...
//                    }
                ...
                if (count == 1) {
                    ...
                    // 走到这里来，应该是代码出问题了，目前按照逻辑来看，是不可能走到这�?�?
                }
            }
        }
    }
    ...
    //更新加入适配器
    private FlowLayoutCallback flowLayoutCallback;
     ...
     * @param callback
     */
    public void setAdapter(FlowLayoutCallback callback) {
        flowLayoutCallback = callback;
    }
    ...
    public void updateLayout() {
        ...
        if (flowLayoutCallback != null) {
            ...
            for (int i = 0; i < size; i++) {
                addView(flowLayoutCallback.getView(i, this), new FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
            }
        }
    }
}
